...
if (is_array($arrUser) && count($arrUser) > 0){
    ...
    if ((int)$arrUser['tiputilizator'] == 2){
        $arrInterviuri = $this->DATABASE->RunQuickSelect('*', SYSCFG_DB_PREFIX . 'interviuri',
            array(
                array('idxauthuniversitate', '=', $arrUser['idx'], 'AND'),
            ...
            foreach ($arrInterviuri as $arrInterviu){
                ...
                if ((int)$arrInterviu['idxobject'] > 0){
                    ...
                    if (is_array($arrLocMunca) && !empty($arrLocMunca)){
                        $nExistaLocUniversitate = 1;
                        $strFacultate = $arrLocMunca[0]['facultate'];
                        $strDomeniu = $arrLocMunca[0]['domeniu'];
                        $strNrLocuri = $arrLocMunca[0]['numarlocuri'];
                        $nIdxLocUniv = (int)$arrLocMunca[0]['idx'];
                    }
                }
                ...
                if (is_array($arrAngajat) && !empty($arrAngajat)){
                    ...
                    $this->DATA['locuri'][] = array(
                        ...
                        'existaloc' => $nExistaLocUniversitate,
                        'facultate' => $strFacultate,
                        'idxlocuniversitate' => $nIdxLocUniv,
                        
                        'vonagesessid' => $arrInterviu['vonagesessid'],
                        'token' => $arrInterviu['vonageinterlocutortoken']
                        //'domeniu' => $strDomeniu,
                        //'nrlocuri' => $strNrLocuri
                    );
