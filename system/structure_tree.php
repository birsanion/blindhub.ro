...
define('SYSCAP_TREE_STRUCTURE',   true);

define('CQTS_NOLANG',    0);
define('CQTS_LANG',      1);

class CQTreeStructure
{
    // variables
    ...
    {
        if (!$this->bIsInit){
            $this->strDBTableNoLang= SYSCFG_DB_PREFIX.'h_'.$arrSettings['rootcode'].'_nolang';
            $this->strDBTableLang=   SYSCFG_DB_PREFIX.'h_'.$arrSettings['rootcode'].'_lang';
    ...
    function AddElement($arrElem, &$nNewId)
    {
        ...
        if ($this->bStoredInDB && $this->DATABASE !== NULL){
            ...
            $arrColumnList=array('parentidx');
            foreach ($this->arrNoLangDetails as $pcDetail) $arrColumnList[]=$pcDetail;
            
            $arrData=array();
            $arrData['parentidx']=$arrElem['nolang']['parentidx'];
    ...
    function DeleteWholeBranch($nRootId)
    {
        if ($this->bStoredInDB && $this->DATABASE !== NULL){
            $arrIdList = array($nRootId);
            $i = 0;
            
            do {
                $arrData = $this->DATABASE->RunQuickSelect('idx', $this->strDBTableNoLang, array('parentidx', '=', $arrIdList[$i]));
                
                if (is_array($arrData) && !empty($arrData))
                    foreach ($arrData as $arrDataSingle)
                        $arrIdList[] = (int)$arrDataSingle['idx'];
                
                $i++;
            }while($i < count($arrIdList));
            
            // TODO: switch to something "cacheable"
            $this->DATABASE->RunQuery('DELETE FROM `' . $this->strDBTableNoLang .'` WHERE `idx` IN(' .
                implode(', ', $arrIdList) . ')');
                
            $this->DATABASE->RunQuery('DELETE FROM `' . $this->strDBTableLang .'` WHERE `nolangidx` IN(' .
                implode(', ', $arrIdList) . ')');
        }
    }
    ...
    function GetElementListSingle($mxFilterNoLang=NULL, $mxFilterLang=NULL, $arrOrderByNoLang=NULL, $arrOrderByLang=NULL)
    {
        if ($this->bStoredInDB && $this->DATABASE !== NULL){
            $arrRet = array();
            ...
            foreach ($arrRawNoLang as $arrItemNoLang){
                ...
                if (!empty($arrLangData) || (empty($arrLangData) && $mxFilterLang===NULL)){
                    ...
                    $arrCurrentItem['nolang']['parentidx']=$arrItemNoLang['parentidx'];
    ...
    function GetElementListRecursive($mxFilterNoLang=NULL, $mxFilterLang=NULL)
    {
        if ($this->bStoredInDB && $this->DATABASE !== NULL){
            $arrRet=array();
            
            // get raw data
            $arrRawNoLang = $this->DATABASE->RunQuickSelect('*', $this->strDBTableNoLang, $mxFilterNoLang, array('parentidx', 'idx'));
            
            $arrRawLang = array();
            
            if ($this->arrLanguages !== NULL)
                $arrRawLang = $this->DATABASE->RunQuickSelect('*', $this->strDBTableLang, $mxFilterLang);
            ...
            foreach ($arrRawNoLang as $arrItemNoLang){
                ...
                if (!empty($arrLangData) || (empty($arrLangData) && $mxFilterLang === NULL)){
                    ...
                    foreach ($this->arrNoLangDetails as $pcDetail)
                        $arrCurrentItem['nolang'][$pcDetail] = $arrItemNoLang[$pcDetail];
                    ...
                    // get subitems
                    if ($mxFilterNoLang !== NULL){
                        $arrSubitems = $this->GetElementListRecursive(array('parentidx', '=', $arrCurrentItem['nolang']['idx']));
                        
                        if (!empty($arrSubitems)) foreach ($arrSubitems as $arrSubitem) $arrRet[] = $arrSubitem;
                    }
                }
            }
    ...
    function GetElementStructuredList($mxFilterNoLang=NULL, $mxFilterLang=NULL)
    {
        if ($this->bStoredInDB && $this->DATABASE !== NULL){
            $arrRet=array();
            ...
            foreach ($arrRawNoLang as $arrItemNoLang){
                ...
                if (!empty($arrLangData) || (empty($arrLangData) && $mxFilterLang === NULL)){
                    ...
                    // get subitems
                    if ($mxFilterNoLang !== NULL){
                        $arrSubitems = $this->GetElementStructuredList(array('parentidx', '=', $arrCurrentItem['nolang']['idx']));
                        
                        if (!empty($arrSubitems)){
                            $arrCurrentItem['children'] = array();
                            
                            foreach ($arrSubitems as $arrSubitem) $arrCurrentItem['children'][] = $arrSubitem;
                        }else $arrCurrentItem['children'] = NULL;
                    }
